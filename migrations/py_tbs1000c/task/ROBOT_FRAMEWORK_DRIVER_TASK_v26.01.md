...
new typed Python core driver (`py_tbs1000c`) that owns SCPI framing, transport, and waveform
...
├── py_tbs1000c/                new typed core driver: transport, SCPI, waveform decode
...
2. **Typed Python driver layer** (`py_tbs1000c`) — SCPI formatting/parsing, channel/trigger/
...
- Robot keywords shall not duplicate SCPI formatting logic; that belongs in `py_tbs1000c`.
...
- Every "save X to a file" keyword that touches the instrument's own filesystem
  ...
  path the caller asked for — implement this once in `py_tbs1000c`, not per keyword.
