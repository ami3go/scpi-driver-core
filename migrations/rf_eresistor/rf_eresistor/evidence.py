...
Relationship to ``eresistor_driver.logging_utils.AuditLogger``
----------------------------------------------------------------
:mod:`eresistor_driver` already ships a framework-agnostic audit facility
...
``eresistor_driver`` itself is never modified. It is also used standalone
(``eresistor_driver/cli.py``), so it must stay framework-agnostic; RFDS-008
...
def _classify_exception(exc: BaseException) -> str:
    ...
    (``eresistor_driver/exceptions.py``); RFDS-007 codes were not available
...
class EvidenceRun:
    ...
    def log_protocol(
        ...
        *,
        ...
        ``rf_eresistor/library.py``; ``eresistor_driver`` itself is untouched.
        """
