...
replaced. These drive a real ``Agilent34411A`` over the real ``PyvisaTransport``
over the core's ``VisaTransport``, against a fake VISA backend.

This is what section 42A of the core's task document asks for.
"""
...
from py_agilent34411a.driver import Agilent34411A
from py_agilent34411a.exceptions import Agilent34411AConnectionError, Agilent34411ATimeoutError
from py_agilent34411a.transport import PyvisaTransport
from scpi_driver_core import ScpiClient
from scpi_driver_core.transport import VisaTransport

IDN = b"Agilent Technologies,34411A,MY47001234,2.35-2.35-0.09-46-09\n"
...
def _answer(command: str) -> bytes:
    replies = {
        ...
        "READ?": b"+1.04858000E+00,+1.04859000E+00\n",
        "FETCh?": b"+1.04858000E+00,+1.04859000E+00\n",
        "FUNCtion?": b'"VOLT"\n',
    }
...
def test_timeout_becomes_the_drivers_own_error(fake_visa: None) -> None:
    ...
    transport.open()
    try:
        with pytest.raises(Agilent34411ATimeoutError):
...
def test_driver_identifies_over_visa(fake_visa: None) -> None:
    driver = Agilent34411A.connect_visa("GPIB0::22::INSTR")
    try:
        identity = driver.identify()
        assert identity.manufacturer == "Agilent Technologies"
        assert identity.model == "34411A"
        assert identity.serial == "MY47001234"
        assert identity.firmware == "2.35-2.35-0.09-46-09"
    finally:
        driver.close()


def test_driver_reads_a_measurement_burst_over_visa(fake_visa: None) -> None:
    driver = Agilent34411A.connect_visa("GPIB0::22::INSTR")
    try:
        assert driver.get_reading() == [1.04858, 1.04859]
    finally:
        driver.close()
...
def test_identity_tolerates_a_partial_reply() -> None:
    ...
    from py_agilent34411a.driver import _identity_fields
...
def test_identity_uses_the_core_for_well_formed_replies() -> None:
    ...
    fields = _identity_fields("Agilent Technologies,34411A,MY47001234,2.35")
    assert fields["model"] == "34411A"
    assert fields["serial"] == "MY47001234"


def test_readings_parse_through_the_core() -> None:
    from py_agilent34411a.driver import Agilent34411A as Driver

    assert Driver._parse_readings("+1.0,+2.0, +3.0 ") == [1.0, 2.0, 3.0]
    assert Driver._parse_readings("") == []
