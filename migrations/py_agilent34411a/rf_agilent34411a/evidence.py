...
``py_agilent34411a/transport.py``) and logs every ``write``/``query`` call
...
def _classify_exception(exc: BaseException) -> str:
    ...
    Maps ``py_agilent34411a/exceptions.py`` names to a small stable category set;
...
class EvidenceRun:
    ...
    def _write_environment(self) -> None:
        payload = {
            ...
            "driver_source_version": _safe_module_version("py_agilent34411a"),
...
class InstrumentedTransport:
    """Wraps any ``py_agilent34411a.transport.Transport`` and traces every write/query.
