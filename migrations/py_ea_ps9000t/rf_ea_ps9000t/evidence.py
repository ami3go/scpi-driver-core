...
def _classify_exception(exc: BaseException) -> str:
    ...
    maps the driver's own exception names (``py_ea_ps9000t/exceptions.py``) to a
...
class TracingTransport:
    """Wraps any ``py_ea_ps9000t.transport.Transport`` and logs every SCPI
    ...
    ``py_ea_ps9000t.driver`` — this class only observes the text that crosses
...
class EvidenceRun:
    ...
    def _write_environment(self) -> None:
        payload = {
            ...
            "driver_source_version": _safe_module_version("py_ea_ps9000t"),
