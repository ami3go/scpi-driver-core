...
```text
        Robot keywords / pytest fixtures
                      |
              Keysight N6700 driver        device semantics, safety, verdicts
                      |
                 ScpiClient           SCPI framing, typed queries
                      |
              VisaTransport and TcpTransport
```
...
the hand-rolled PyVISA and raw-socket transports, and the identity, SYST:ERR?, CSV and definite-length-block parsers.

## What stayed here

the (@1:4) channel grammar, the manufacturer whitelist, the comma-separated multi-block framing, and the REAL64 datalog unpacking.
...
Three defects went with the old code. VISA terminations were left on, so PyVISA could trim bytes out of a binary block. The raw-socket read looped recv until a newline with no maximum size. And the transports opened in __init__ with no state model, so a failed connection and a closed one were indistinguishable.
