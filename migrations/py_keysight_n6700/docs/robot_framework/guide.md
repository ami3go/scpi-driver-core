...
`KeysightN6700Library` is an adapter over the existing `py_keysight_n6700` driver. It does not reimplement SCPI commands. This separation keeps hardware policy, module capability checks, transport behavior, and simulator behavior in one tested driver layer.
