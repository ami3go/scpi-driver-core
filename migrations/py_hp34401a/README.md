...
| Core driver | `py_hp34401a_dmm` 1.2.8 plus RFDS runtime-policy facade |
...
The public Robot adapter delegates SCPI measurement behavior to the reviewed `py_hp34401a_dmm` core. It rejects overload, invalid, missing, and unstable readings instead of returning plausible fabricated values. Hardware connection never silently falls back to simulation.
...
- `py_hp34401a_dmm/driver.py` — active core runtime-policy facade;
- `py_hp34401a_dmm/legacy_driver.py` — preserved reviewed 1.2.8 SCPI implementation;
...
being implemented here. HP 34401A speaks VISA GPIB, and all of that is the core's
byte-oriented transport layer with this package's device semantics on top.

What changed in the migration: py_hp34401a_dmm/visa_transport.py. Everything else — the command tree,
