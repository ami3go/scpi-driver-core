"""RFDS-008 evidence and diagnostics engine for :mod:`rf_eresistor`.

Every public keyword call is recorded as a *run* under ``results/session/
rf_eresistor/<timestamp>_<run_id>/`` (override the root with the
``RFDS_EVIDENCE_ROOT`` environment variable): one JSON Lines entry per
event/operation/error, a redacted argument record, the underlying wire
protocol exchanges as a protocol trace, a device-identity snapshot, and, once
``Disconnect From EResistor`` runs, a ``run_summary.json`` +
``evidence_manifest.json`` pair with a SHA-256 hash per artifact.
``Export Diagnostic Bundle`` zips the whole thing up for troubleshooting.

Relationship to ``py_eresistor_driver.logging_utils.AuditLogger``
----------------------------------------------------------------
:mod:`py_eresistor_driver` already ships a framework-agnostic audit facility
(``AuditLogger.log_command``), wired into ``EResistorClient.query()`` and
enabled via the ``audit_log_file``/profile constructor argument. That
facility is unchanged by this module and keeps working exactly as before —
it logs only *state-changing* SCPI commands, as a single-line JSON-per-line
file, with no manifest/hashing/correlation/run-summary concept. This module
is a deeper, Robot-Framework-side complementary layer: it traces *every*
SCPI exchange (not just state-changing ones) and every HTTP call, correlates
them with the exact keyword invocation and arguments that caused them, and
produces integrity-checked, machine-validated evidence for troubleshooting a
specific test run. Deliberately does not read or duplicate the audit log
file; the two coexist. See ``docs/logging_and_evidence.md``.

Protocol tracing is implemented entirely at the RF adapter layer (this
package), by wrapping ``EResistorClient.query`` and
``EResistorClient.http.get_text`` on the *instance* after connecting —
``py_eresistor_driver`` itself is never modified. It is also used standalone
(``py_eresistor_driver/cli.py``), so it must stay framework-agnostic; RFDS-008
evidence is a Robot-Framework/keyword-shaped concern and belongs here.
...
redacted evidence with integrity hashes — and does not attempt platform-scale
concerns the standard also describes (log rotation/backpressure policy,
crash-recovery tooling, cryptographic signing, retention/archival automation,
a cross-driver shared package). Those are noted inline where relevant rather
than silently skipped. This is a per-driver copy of the same design used by
``rf_phidget_relay/rf_phidget_relay/evidence.py`` and
``rf_ea_ps9000t``-style drivers; no shared core package exists yet in this
repository (RFDS-008 §35 "should", not "shall").
"""
...
logger = logging.getLogger("rf_eresistor.evidence")
...
def _classify_exception(exc: BaseException) -> str:
    ...
    (``py_eresistor_driver/exceptions.py``); RFDS-007 codes were not available
    while writing this, so exception class names are mapped to a small
    stable category set instead."""
    name = type(exc).__name__
    if name in ("ConnectionError",) or "Connection" in name:
        return "STATE"
    if name in ("TimeoutError",) or "Timeout" in name:
        return "HARDWARE"
    if name in ("ScpiError", "HttpApiError"):
        return "HARDWARE"
    if name in ("CalibrationError", "SimulationError"):
        return "VALIDATION"
...
def _safe_distribution_version(dist_name: str) -> str | None:
    ...
    except Exception:  # noqa: BLE001 - environment introspection must never break evidence recording
        return None
...
def _safe_call(obj: Any, method_name: str) -> Any:
    """Best-effort vendor/device query; ``None`` (never a raised exception) when unavailable."""
    try:
        return getattr(obj, method_name)()
    except Exception:  # noqa: BLE001 - best-effort device query; caller treats None as UNKNOWN, never crashes
        return None
...
class EvidenceRun:
    ...
    def _write_environment(self) -> None:
        payload = {
            ...
            "driver_distribution_version": _safe_distribution_version("rf-eresistor"),
            "driver_source_version": _safe_module_version("rf_eresistor"),
            "core_driver_distribution_version": _safe_distribution_version("eresistor-driver"),
    ...
    def emit_event(
        ...
        *,
        ...
        record = {
            ...
            "schema_version": SCHEMA_VERSION,
            "run_id": self.run_id,
            "producer_id": f"rf_eresistor:{session_alias or self.driver_id}",
            "timestamp_utc": _utc_now_iso(),
        ...
        # ERROR/CRITICAL are not counted here: every current ERROR/CRITICAL emit_event
        # call is paired with a record_error() call, which solely owns _error_count.
    ...
    def log_protocol(
        ...
        *,
        ...
        """Record one outbound/inbound wire exchange as protocol evidence.

        ``transport`` is ``"scpi_tcp"`` (via ``EResistorClient.query()``) or
        ``"http"`` (via ``EResistorHttpApi.get_text()``) for this driver —
        both are traced by wrapping the connected client instance in
        ``rf_eresistor/library.py``; ``py_eresistor_driver`` itself is untouched.
        """
    ...
    def _write_markdown_summary(self, summary: dict) -> None:
        lines = [
            ...
            "for the underlying SCPI/HTTP exchanges, in the order they happened.",
            "",
        ]
...
class EvidenceListener:
    """Register with ``--listener rf_eresistor.evidence.EvidenceListener`` to
    correlate events with the active Robot suite/test/keyword. Optional —
    the evidence engine works the same without it."""
