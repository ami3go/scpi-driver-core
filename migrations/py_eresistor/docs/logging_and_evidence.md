...
`py_eresistor_driver` already ships a framework-agnostic audit facility
(`AuditLogger`, in `py_eresistor_driver/logging_utils.py`), enabled via the
...
outside Robot Framework (`py_eresistor_driver/cli.py`).
