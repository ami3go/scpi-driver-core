...
``Transport.query()`` boundary (see ``py_agilent33220a/transport.py``) — so
...
scattered through every keyword. The core driver (``py_agilent33220a/``) never
...
def _classify_exception(exc: BaseException) -> str:
    ...
    ``py_agilent33220a/exceptions.py``'s own names to a small stable category
...
# ---------------------------------------------------------------------------
# Protocol tracing: wraps any py_agilent33220a.transport.Transport
# ---------------------------------------------------------------------------
...
class TracingTransport:
    ...
    ``timeout_s``) so it's a drop-in replacement — ``py_agilent33220a/driver.py``
...
class EvidenceRun:
    ...
    def _write_environment(self) -> None:
        payload = {
            ...
            "driver_source_version": _safe_module_version("py_agilent33220a"),
