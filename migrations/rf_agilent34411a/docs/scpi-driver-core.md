...
```text
        Robot keywords / pytest fixtures
                      |
              Agilent 34411A driver        device semantics, safety, verdicts
                      |
...
the hand-rolled PyVISA transport, and the identity and reading-burst parsing.

## What stayed here

the measurement-function semantics, the overload sentinel policy, and the reading-memory behaviour.
...
The transport module no longer imports pyvisa at all, so the package rule that protocol code never touches pyvisa outside it now holds one level more strongly. Identity parsing keeps this driver's tolerance for partial *IDN? replies, where missing fields become empty strings rather than raising.
